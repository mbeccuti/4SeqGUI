...
public class S_DenseToSparse extends javax.swing.JPanel {
    ...
    private void S_LorenzFilter_jButton39ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_S_LorenzFilter_jButton39ActionPerformed
        //Field check
        String scratch = sScratchFolder.getText(), 
               input = S_countTable.getText(); 
        
        if (MainFrame.checkPath(this, scratch, "scratch folder") || 
            MainFrame.checkPath(this, input, "counts table file"))
            return; 
        
        String outputFolder = Paths.get(input).getParent().toString();
        ScriptCaller params = new ScriptCaller("denseToSparse.R", outputFolder)
                .addArg("group", cSudoRadioButton.isSelected() ? "sudo" : "docker")
                .addArg("scratch.folder", scratch)
                .addArg("file", input)
                .addArg("separator", S_separator.getSelectedItem().toString()); 
        MainFrame.execCommand(this, "Dense To Sparse", params);
